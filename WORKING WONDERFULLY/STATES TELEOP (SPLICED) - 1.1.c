...
#pragma config(Motor,  mtr_S1_C1_1,     armunused,     tmotorTetrix, PIDControl, reversed, encoder)
...
task main()
{
	...
	int joylevels[4]={0,300,500,1000};
	...
	while (true)
	{
		...
		if (joy_2y1!=0){
			...
			if (tophat==0 && roundup != -1){//up
				...
				writeDebugStreamLine("SET MOTOR SPEED to %d from %d", 20 , motor[arm] );
				motor[arm]=20;
			}
