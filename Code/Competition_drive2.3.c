...
#pragma config(Servo,  srvo_S1_C3_1,    sarm,                 tServoStandard)
#pragma config(Servo,  srvo_S1_C3_2,    bottle,               tServoStandard)
#pragma config(Servo,  srvo_S1_C3_3,    grabber,              tServoStandard)
...
//FUNCTION TO HANDLE JOYSTICK TO MOTOR TRANSLATION
// if abs(joystick) is below deadband and that it doesn't jump after it leaves the deadband
...
void init(){
	//ENCODER SETUPS
	nMotorEncoder[arm]=0;//initiating encoders

	//SERVO SETUPS
	//	SERVO SPEEDS
	//servoChangeRate[sarm]=sec_arm_speed;//setting rate for servos
	//servoChangeRate[bottle]=sec_arm_speed;//setting rate for servos
}


// FUNCTION FOR MOVEMENT OF MAIN ARM
...
while ((servo[sarm]-5>servo_levels[*rounddown])
&& (servo[sarm]+5<servo_levels[*rounddown]) ){
servo[bottle]=servo[sarm];
}

break;
case 4://down
if (*rounddown==0){//check if it is going back to lowest level (move servo first)
...
task main()
{
	...
	while (true)
	{
		...
		joy_1y1=transfer_J_To_M(-joystick.joy1_y1, joyslope, dband);//joystick 1 is dedicated to arm
		...
		tophat=joystick.joy1_TopHat;//shortcut to joystick 1 tophat
		joy_2y1=transfer_J_To_M(-joystick.joy2_y1, joyslope*(float)1./2., dband);//joystick 2 is deditcated to drive
		joy_2y2=transfer_J_To_M(-joystick.joy2_y2, joyslope*(float)1./2., dband);
		...
		/*
		// ARM MANUAL CONTROL CODE
		if (joy_1y2!=0 || joy_1y1!=0)//testing for joy being used
		{
		if (joy_1y1>0) servo[sarm]=servo[sarm]+1;//if the joy is up then increase the angle of bottle by 1
		else if (joy_1y1<0) servo[sarm]=servo[sarm]-1;//opposite


		if (joy_1x1>0) servo[bottle]=servo[bottle]+1;//if the joy is right then increase the angle of bottle by 1
		else if (joy_1x1<0) servo[bottle]=servo[bottle]-1;//opposite
