...
int armrot[3]={0,0,0};//arm rotations for different levels (can be called by 0,1,2 0 being ground and 2 being highest)
...
void liftmovement(/*int currpos, int tophat*/){//I shouldn't need to have an input for them because i initialised them at top
	...
	switch (tophat){
		case 0://if tophat is up
			if (currpos==sizeof(armrot)/4-1)//if on roof then don't go up
				...
				motor[arm]=armrot[currpos+1]-armrot[currpos];
				currpos++;
			}
			break;
		...
		case 4://if tophat is down
			if (currpos==0)//if on floor then don't go down
				...
				motor[arm]=armrot[currpos-1]-armrot[currpos];//had to decrease because of the number
				currpos--;
			}
			break;
	}
	...
	/*
	if (tophat==4 && currpos==0){//checks to stop down movement when on floor
		writeDebugStreamLine("CAN'T MOVE DOWN!");
	}
	else if (tophat==0 && currpos)
	{
		writeDebugStreamLine("CAN'T MOVE UP!");
	}
	else
	{
		writeDebugStreamLine("Perfect!");
		if ()
	}
	*/
}
